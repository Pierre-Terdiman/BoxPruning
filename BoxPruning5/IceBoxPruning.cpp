...
bool Meshmerizer::BipartiteBoxPruning(udword nb0, const AABB** list0, udword nb1, const AABB** list1, Container& pairs)
{
	...
	for(udword i=0;i<nb0;i++)
		MinPosList0[i] = list0[i]->mMin.x;
	MinPosList0[nb0] = FLT_MAX;
	for(udword i=0;i<nb1;i++)
		MinPosList1[i] = list1[i]->mMin.x;
	...
	while(RunningAddress1<LastSorted1 && Sorted0<LastSorted0)
	{
		...
		const float MaxLimit = list0[Index0]->mMax.x;
		while(MinPosList1[Index1 = *RunningAddress2_1++]<=MaxLimit)
		{
			if(intersects2D(*list0[Index0], *list1[Index1]))
			{
	...
	while(RunningAddress0<LastSorted0 && Sorted1<LastSorted1)
	{
		...
		const float MaxLimit = list1[Index0]->mMax.x;
		while(MinPosList0[Index1 = *RunningAddress2_0++]<=MaxLimit)
		{
			if(intersects2D(*list0[Index1], *list1[Index0]))
			{
...
bool Meshmerizer::CompleteBoxPruning(udword nb, const AABB** list, Container& pairs)
{
	...
	// Catch axes
//	const udword Axis0 = axes.Axis0;	// REMOVED
//	const udword Axis1 = axes.Axis1;	// REMOVED
//	const udword Axis2 = axes.Axis2;	// REMOVED
...
//		PosList[i] = list[i]->GetMin(Axis0);	// MODIFIED
		PosList[i] = list[i]->mMin.x;			// MODIFIED
	PosList[nb] = FLT_MAX;
