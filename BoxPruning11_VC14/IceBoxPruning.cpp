...
bool Meshmerizer::BipartiteBoxPruning(udword nb0, const AABB* list0, udword nb1, const AABB* list1, Container& pairs)
{
	...
	udword* Remap1;
	{
		...
		udword* Sorted0 = RS0.Sort(PosList0, nb0 + 1).GetRanks();
		udword* Sorted1 = RS1.Sort(PosList1, nb1 + 1).GetRanks();

		Remap0 = RS0.GetRecyclable();
		Remap1 = RS1.GetRecyclable();

		for (udword i = 0; i<nb0; i++)
		{
			const udword SortedIndex = Sorted0[i];
			Remap0[i] = SortedIndex;
			BoxList0[i] = list0[SortedIndex];
		}
		BoxList0[nb0].mMin.x = FLT_MAX;

		for (udword i = 0; i<nb1; i++)
		{
			const udword SortedIndex = Sorted1[i];
			Remap1[i] = SortedIndex;
			BoxList1[i] = list1[SortedIndex];
		}
