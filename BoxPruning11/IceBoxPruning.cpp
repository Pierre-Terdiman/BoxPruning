...
bool Meshmerizer::CompleteBoxPruning(udword nb, const AABB* list, Container& pairs)
{
	...
	while(RunningAddress<nb && Index0<nb)
	{
		...
		// If RunningAddress==nb here, then Index1==nb and BoxListX[Index1].mMinX will be FLT_MAX (the sentinel value).
		// The test against MaxLimit will always fail, unless MaxLimit is also FLT_MAX. That can only happen if Index0==nb as well.
		// And that cannot happen because the first while tests that "Index0<nb".
//		if(RunningAddress==nb)
//			int stop=0;
