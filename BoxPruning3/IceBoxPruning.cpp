...
bool Meshmerizer::BipartiteBoxPruning(udword nb0, const AABB** list0, udword nb1, const AABB** list1, Container& pairs, const Axes& axes)
{
	...
	while(RunningAddress1<LastSorted1 && Sorted0<LastSorted0)
	{
		...
		while(RunningAddress1<LastSorted1 && MinPosList1[*RunningAddress1]<MinLimit)
			RunningAddress1++;

		const udword* RunningAddress2_1 = RunningAddress1;

		const float MaxLimit = list0[Index0]->GetMax(Axis0);
		while(RunningAddress2_1<LastSorted1 && MinPosList1[Index1 = *RunningAddress2_1++]<=MaxLimit)
		{
	...
	while(RunningAddress0<LastSorted0 && Sorted1<LastSorted1)
	{
		...
		while(RunningAddress0<LastSorted0 && MinPosList0[*RunningAddress0]<=MinLimit)
			RunningAddress0++;

		const udword* RunningAddress2_0 = RunningAddress0;

		const float MaxLimit = list1[Index0]->GetMax(Axis0);
		while(RunningAddress2_0<LastSorted0 && MinPosList0[Index1 = *RunningAddress2_0++]<=MaxLimit)
		{
...
//CPUID
	...
	while(RunningAddress<LastSorted && Sorted<LastSorted)
	{
		...
		const float MinLimit = PosList[Index0];	// NEW
